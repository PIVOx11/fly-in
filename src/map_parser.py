...
class Parser:
    ...
    def generate_graph(self) -> Graph:
        ...
        for line_c, line in enumerate(self.file, start=1):
            ...
            if line_c - comment > 1 and not self.graph.drone_count:
                raise ParsingError(
                    "dron_nb not At the begenning of the file .")
            if line[0] in handlers:
                handlers[line[0]](line, line_c)
            else:
                raise ParsingError(
                    f"Line {line_c} Unknown Key Argument '{line[0]}'")
            comment += 1
    ...
    def hubs_parser(self, line: list[str], line_c: int) -> None:
        ...
        if len(arg) not in (3, 4):
            raise ParsingError(
                f"Line {line_c}: Data Counte Invalide .")
        name, x, y, metadata = \
            arg[0], arg[1], arg[2], None if len(arg) == 3 else arg[3]
    ...
    def name_check(self, name: str, line_c: int) -> str:
        ...
        if name.isdigit():
            raise ParsingError(f"Line {line_c}: "
                               ...
                               ", example('_path1', 'maze_1') "
                               f"are valid but {name} NOT .")
        if "-" in name:
            raise ParsingError(f"Line {line_c}: "
                               "Zone Name Contain Dash '-'"
                               " , example('path1', 'maze_1') "
                               f"are valid but {name} NOT .")
        return name
    ...
    def cordonate_validation(self, x: int, y: int, line_c: int) -> tuple:
        ...
        if (cord := (x, y)) in self.coordinates:
            raise ParsingError(
                f"Line {line_c}: Coordinates duplecated {cord}.")
        ...
        try:
            for string in metadata:
                ...
                if k in allowed_data:
                    if k in data:
                        raise ParsingError(
                            f"Line {line_c}: duplacated Data '{k}' .")
                    if k == "zone" and v not in allowed_data[k]:
                        raise ParsingError(f"Line {line_c}: "
                                           f"Unkown Zone type '{v}', "
                                           "allowed Zonetypes: "
                    ...
                    if k == "max_drones":
                        if v.isdigit():
                            ...
                            if v <= 0:
                                raise ParsingError(
                                    f"Line {line_c}: "
                                    "Max dron Should be valid integer (1-45).")
                        else:
                            raise ParsingError(
                                f"Line {line_c}: "
    ...
    def connection_handler(self, line, line_c):
        ...
        if len(line) not in (1, 2):
            raise ParsingError(
                f"Line {line_c}: Wrong Connection Data format .\n"
                "Example: onnection: <name1>-<name2> [metadata]")
        zones = line[0].strip().split("-", 1)
        if len(zones) != 2:
            raise ParsingError(
                f"Line {line_c}: Wrong Connection Between Zones .\n"
                "Example: <name1>-<name2>")
        zones.sort()
        zones = tuple(zones)
        if zones in self.connections:
            raise ParsingError(
                f"Line {line_c}: Connection between "
                f"{'-'.join(zones)} Are Duplacated :) .")
        ...
        if len(line) == 2:
            ...
            if len(metadata) != 2:
                raise ParsingError(
                    f"Line {line_c}: "
                    "metadata invalid only drones capacity allowed .")
            k, v = metadata
    ...
    def create_connections(self):
        for zones, capacity in self.connections.items():
            zone1, zone2 = self.graph.zones.get(zones[0]), \
                self.graph.zones.get(zones[1])
